...
async function parse(
  ...
  try {
    const result = await new Promise<unknown>((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error("Parser deadline")), 5000);
...
describe("pinned source-built Nodestone under Bun", () => {
  test("FC identity is lossless and explicit zero differs from missing count", async () => {
  ...
  test("profile operation preserves biography and canonical source attributes", async () => {
  ...
  test("members return entries and complete pagination fields", async () => {
    const result = await parse(
  ...
  test("search encodes once and appends pagination correctly", async () => {
  ...
  test("missing roster root is rejected rather than accepted as empty", async () => {
    await expect(
      parse({ operation: "members", id: fcId, page: 1 }, "<html>Maintenance</html>"),
    ).rejects.toThrow("invalid_response");
  });
  test("transport categories and retry metadata survive the parser boundary", async () => {
    for (const code of ["not_found", "rate_limited", "unavailable"]) {
      await expect(
        parse({ operation: "fc", id: fcId }, { code, retryAfter: code === "rate_limited" ? 7 : 0 }),
