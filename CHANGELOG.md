...
- **What goes public** (`src/domain/suggestions.ts`, pure). Only the cleaned text and TaruBot's version: a fixed first line, the text in a `text` code block, and "Sent by TaruBot X.Y.Z." The title is the start of the text, cut at a word within 80 characters.
  ...
  - `clean` applies one shared list, repeated until nothing changes (nested Discord markup unwraps one level per pass): Discord mentions become words, custom emoji and command mentions keep their names, links with or without a scheme become `[link removed]`, then email addresses, the issue reporter's credential shapes, runs of 17 or more digits in any script, and every `@` (to `＠`). Titles also turn `#` into `＃` and `GH-` into a look-alike, so nothing cross-references.
