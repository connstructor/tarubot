...
- Router: handlers receive the viewer and return presenter replies (legacy options stay accepted until every group migrates); a component can update its own message in place, only when that message is private or belongs to the presser; the interaction scope (for example `/ledger withdraw` or `button guest`) is logged with each failure; expired interactions and failures while sending a reply are logged at warn instead of rejecting.
