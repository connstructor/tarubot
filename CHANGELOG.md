...
- **Startup.** Each present server with a channel and an older baseline gets one `changelog.post` job (`changelog:<guild>`, payload `{}`), whatever the effects switches say. A restart while one is pending merges into it, so a post covers every release since the last. The log line "Queued update posts" carries the count and the version only.
