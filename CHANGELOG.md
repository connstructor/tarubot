...
- **No channel** (decision 5: changes made while it is unset aren't saved for later). A pass reads the officer notifications channel under its guild lock: with none, its change is taken as announced at once and nothing is queued; the roster likewise records no departure (and locks no owner) in a guild without one. So setting the channel within two minutes of a change never posts it. Every pass with no channel also clears whatever the member still has waiting from before the unset, `status_posting` included (what the job's no-channel drop clears), whether or not it changed anything: a job that ended `failed` before the unset is never revived, so without this its waiting change and frozen batch would post once a channel was set again. A pass with nothing to clear still writes nothing.
