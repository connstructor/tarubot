...
- **Workflow.** `.github/workflows/deploy.yml` ("Deploy production") runs after "Publish containers" succeeds for a push to main, or by hand with a version (a rollback also names the live version it leaves). Its three jobs use no action and no checkout, and every expression reaches a script through `env:`:
  - `plan` (a read-only token, no environment, no secret) checks that `production` requires @deconfined alone with self-review allowed, no admin bypass and main only, and that `notify` accepts only main. It resolves the version, commit and image digest (the version tag and `sha-<commit>` must be the same image, on a commit on main) and decides whether anything that runs in production changed. It writes the plan to the run summary: migrations added, host-side changes, warnings for the Tuesday maintenance window and the daily backup, the changelog section and what approving does. Docs-, test- and CI-only merges end there with a quiet Pushover message.
  - `deploy` runs in the `production` environment, so it waits for @deconfined's approval. It reads the host from the environment variable `DEPLOY_HOST` (the repository names no host), pins its key from `DEPLOY_KNOWN_HOSTS`, and sends one command to the host's forced command, retrying only dropped connections, with the same command. A changed host key or a rejected key stops it at once.
  - `notify` sends one Pushover message from the `notify` environment: deployed, already live, superseded, not deployed, recovered, or NEEDS YOU with what to do.
...
- **Tests.** `deploy-script.test.ts` runs the real script against a throwaway git repository with simulated `docker`, `curl` and `df`: hostile requests, every path and recovery rule, the refusals, the approval check, attach and replay (with a committed v1 run directory) and the detached launch. `deploy-workflow.test.ts` pins the workflow's shape and its contract with the host script, asserts that neither names a host, and runs the plan, SSH and notify scripts with simulated `gh`, `docker`, `ssh` and `curl`. `migration-files.test.ts` now forbids migrations that control their own transaction, which the recovery rule relies on.
- **Requirements and rules.** REQUIREMENTS.md "Approved SSH-deploy amendments (2026-09-26)" records the owner's ten answers: his approval of the `production` environment in GitHub is the go-ahead for a production deploy and what its plan lists; the host dump stands in for the operator dump in automated migrations (the manual procedure keeps it); docs-, test- and CI-only releases ask nothing; port 22 stays open to all under the firewall; the maintenance window warns; and the agent guard (Claude Code deny rules now, a narrower token later, its permissions itemized in CI_CD.md). CLAUDE.md and AGENTS.md: Claude sessions never approve, reject or bypass a deployment, never create or hold the deploy key, never change the deployment environments, their secrets or variables, and start the workflow only when the owner asks.
- **Docs.** HOSTING.md "Automated deploys": the flow, what the host checks and does, every outcome and reason with what to do, the host's logs, what to do when a worker dies, pause, stop and kill, and the owner's setup (environments, `jq`, the key generated in memory into `production`, its restricted `authorized_keys` line and probes, `DEPLOY_HOST` and `DEPLOY_KNOWN_HOSTS` checked against SSHFP, Pushover, the firewall, `DEPLOY_ENABLED`, a first run expecting `already-live`). The rebuild runbook installs `jq`, checks logind's `KillUserProcesses`, rotates the deploy key and updates `DEPLOY_KNOWN_HOSTS`; the manual procedure gains `-T` and names release commits rather than tags. CI_CD.md "Deploy production" has the triggers, jobs, settings and rules. The records (OPEN_ITEMS.md, SESSION_HANDOFF.md, DEV_GUILD.md's planned rollout, VERIFICATION.md) and the old App Platform proposal point at 2.30.0. The site's deploy pages describe the stock Compose install and are unchanged.
- **Rollout.** 2.30.0 changes no running code, so DevBot needs nothing. Before the merge the owner creates the environments with `DEPLOY_ENABLED` unset, so the first run after the merge does nothing. Production gets 2.30.0 once by hand, as a restart, so the host has `ops/deploy.sh`; then the owner's setup and a first run of 2.30.0. No member note: members notice nothing.
