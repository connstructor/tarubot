...
- **The job.** `officer.status`, key `officer:<guild>:status`, payload `{}`, queued by every recorded change and due one window later:
  - no officer notifications channel: what waits is announced as it is and nothing is saved for later (`{skipped: "officer notifications unconfigured"}`), before the effects gate, even with Discord changes off;
  - with Discord changes paused it parks as `disabled` and posts on resume;
  - a batch frozen earlier is resent first, unchanged, under the same nonce key `status:<batch>`;
  - then it waits (`ordered`, at debug) until the oldest change is two minutes old, or completes `{skipped: "nothing to post"}`;
  - then it freezes batches under its lease, sends each and marks it straight after the send, before the lease check, so a lost lease never leaves a posted batch unmarked. Each send records `started`, then `delivered` or `failed` delivery attempts; missing permissions block it and keep the batch frozen.
  - The job label is "Status notice" ("Status notice posted").
- **Lock order.** The roster now decides every link's transition before any write and locks the departing owners' rows in `(guild_id, user_id)` order (`COLLATE "C"`) before any `characters` row, the order `/unclaim`, `/assign` and the two-404 unlink use. `sync.guild` locks its guild's rows up front in the same order. The status job's freeze, mark and no-channel drop share-lock the guild row first, then the rows in user order.
...
- **Not announced** (decision 7): role changes after `/config roles` or `/setup` binding changes (except officers adopted when a new Officer role replaces one already set), hand edits the bot keeps while evidence is unconfirmed, roles given on joining, people who left the server, nicknames, rank changes that don't change Officer or FC Leader, and FC joins that change no access. Linking an FC to a server whose roles are set up posts its confirmed members as "Guest → Member" after the first roster, the mirror of the unlink post.
- **Member note.** None: the posts go only to the officer notifications channel, so members notice nothing (CLAUDE.md step 2).
- **Deploy.** The migration procedure: stop, back up, `migrate.js`, `up`. No command registration. Rolling back to 2.26.x or earlier isn't possible across the migration (the schema check refuses to start); the ways back are a fix release, a restore, or the manual reversal in docs/OPERATIONS.md "Status notices".
- **Tests:**
