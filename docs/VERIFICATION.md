...
The PostgreSQL cases are written but not yet run; the container run (`test:docker`) is still to do: migration 009's rehearsal, startup's enqueue, merge and parked-post collapse, and delivery (baseline rules, audience lookup including the Community Updates channel, one post per range, repeats and a forged payload, nothing for members, blocked then released by `/config`, unset while waiting, paused, where only a post with something to say parks while "nothing for members" and "already announced" still complete, the compare-and-set, two servers).
