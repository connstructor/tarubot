...
Application and maintenance persistence uses the shared typed ORM boundary. The 2.12.0 source requires `SCHEMA_VERSION=004_guest_application_form.sql`; the preceding migrations remain immutable. DevBot still runs 2.11.1/schema 003, with onboarding enabled at revision 10 and its 11-channel access job succeeded. The current `test-plans/current.json` covers deployment of 004 and human guest-form/approval testing; exhaustive human visibility/recovery checks remain. See [DEV_GUILD.md](DEV_GUILD.md), [PERSISTENCE.md](PERSISTENCE.md), and [SETUP.md](SETUP.md).
