...
Application and maintenance persistence uses the shared typed ORM boundary. The current image requires `SCHEMA_VERSION=003_guild_access.sql`; the preceding migrations remain immutable. The 2.11.1 live onboarding session is described in `test-plans/current.json`; full channel enforcement and the human visibility matrix still require that live check. The preceding 2.10.1 deployment and limited-permission officer-chat creation passed, and a read-only compiled 2.10.2 preflight excluded the community resources successfully. See [DEV_GUILD.md](DEV_GUILD.md), [PERSISTENCE.md](PERSISTENCE.md), and [SETUP.md](SETUP.md).
