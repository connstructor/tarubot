...
The current **2.20.0** source adds no migration and requires `SCHEMA_VERSION=008_issue_reports.sql`, which **2.18.0** added. It is additive and needs no superuser privileges. It adds the `issue_reports` table, one row per report fingerprint:
