...
A reconciliation pass records its decisive flags in its own transaction after the role write: the member's row lock, then (only when a value moved) the guild's `officer:<guild>:status` job, and an unchanged pass writes nothing. The accepted roster records departures in its snapshot transaction, locking the departing owners' rows in `(guild_id, user_id)` order (`COLLATE "C"`) before any `characters` row, the order `/unclaim`, `/assign` and the two-404 unlink use; `sync.guild` locks its guild's rows up front in that order. The job's freeze, mark and no-channel drop share-lock the guild row, then lock rows in user order, and take no jobs-row lock; the freeze checks the job's lease with a plain read. The queue now resets `attempts` when a successful run is requeued for a newer generation. The Drizzle mapping still covers 26 application tables. An older image can't start on schema 010; OPERATIONS.md "Status notices" has the manual reversal.

`009_changelog_channel.sql`, introduced in **2.25.0**, is additive and needs no superuser privileges. It adds two nullable `guilds` columns for update posts:
