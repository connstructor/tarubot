...
- DevBot has run the published **2.14.0** images (revision `0e60f21`) on schema 005 since 2026-09-24 at 02:58:17 UTC, after a stopped-writer backup and an exact restore check (no migration); the commands are registered and match the image (19 roots / 41 paths), and guest reviews go to officer-chat (since revision 11). See [DEV_GUILD.md](DEV_GUILD.md#2140-rollout--2026-09-24). Schema 005 was applied on 2026-09-23 at 20:09:55 UTC in the 2.13.0 rollout, after an exact restore/migration rehearsal ([DEV_GUILD.md](DEV_GUILD.md#2130-rollout--2026-09-23)). Administrator remains off, all four existing roles and officer-chat were reused, and the lobby was created. Full human visibility verification remains.
...
| P2 | Confirm the Claude Code Review fix in 2.14.1 once it is merged. Its own PR gets no real review, because the action skips a PR that changes its workflow. On the next ready, same-repository PR to `main` after the merge (probably 2.15.0), "Check that the review finished" should show `started_in_background` 0, `requested.unset` equal to `spawned` (at least 2) and `completed` equal to `spawned`, and `claude[bot]` should post inline comments or a "No issues found" comment. If the check fails, re-run the job with debug logging for the transcript; if the variable did not reach Claude Code, pass `{"env":{"CLAUDE_CODE_DISABLE_BACKGROUND_TASKS":"1"}}` through the action's `settings` input instead. If a large release PR nears the 30-minute timeout, raise `timeout-minutes`. Optional owner action: delete the logs of runs 35948050785 and 35948499732 (PR #12), which still hold full review transcripts in the public Actions log. | CI only, no requirement. Run 35948050785 ended green in about 38 s with one subagent started in the background and none completed; see [CI_CD.md](CI_CD.md). |
...
| 2.14.0 on DevBot | Deployed and registered on 2026-09-24 (no migration; backup, restore check, registration and the startup plan are recorded in DEV_GUILD.md). Remaining: the owner's reply session in `test-plans/current.json`: compare every reply with the approved mockups, match each error's Ref and Code to the log, read back the ledger posts and the review message, and repeat the change receipts and sync views with `ENABLE_EFFECTS=false` (a restart that needs the owner's go-ahead). | **UX-01–03, LEDGER-08** |
...
3. Replace every JSON reply with the owner-approved embeds (2.14.0). **Merged and published in 2.14.0** (PR #12) and deployed to DevBot; the reply session remains. 2.14.1 fixes the Claude review workflow and the sidecar spacing test. Then complete operational alerting/telemetry (OPS-10/OPS-11, 2.15.0).
