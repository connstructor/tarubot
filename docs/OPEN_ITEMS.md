...
2. **Owner preconditions:** remove the production application from the development guild; confirm where the legacy bot runs and how its supervisor restart is disabled; decide the Officer exceptions for `/officer grant`. **Approved launch amendments**.
