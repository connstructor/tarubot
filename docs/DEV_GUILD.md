...
- **Not recorded:** the DevBot `/suggest` checks from the plan (the preview issue in `deconfined/tarubot-reports`, "You can suggest again later" within the hour, "Check your input" for a short idea, and "FC membership needed" without the Member or Guest role), and whether any server got an update post with 2.28.0's member note (production had no changelog channel at the 2.25.0 rollout).
