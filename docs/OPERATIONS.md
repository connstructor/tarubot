...
**What goes public:** only the member's cleaned text, in a code block under a fixed first line that says it came from Discord, and "Sent by TaruBot X.Y.Z." Links (with or without a scheme, in any script, including a domain followed only by a port, query or fragment), IPv4 addresses, global IPv6 addresses (2000::/3: a first group of four hex digits starting with 2 or 3, and at least two colons; bare or in brackets, with any port, path, query or fragment), Discord mentions, email addresses, credential shapes and runs of 17 or more digits are removed, every `@` becomes `＠`, `#` in titles becomes `＃`, and invisible characters are removed; a final check refuses anything that slipped through, as an unexpected failure with a private issue report. The member's name and ID, server, channel and role IDs, FC and character data, logs and the environment never go public. Names typed freely, IDs deliberately split with visible separators, a host whose `。` sits next to a Chinese or Japanese label or TLD (which reads as the end of a sentence), look-alike dots and slashes with no compatibility form (`discord·gg`, `discord.gg∕x`, which aren't links), IPv4 addresses written as one to three decimal numbers (`127.1`, `2130706433`, which read like ratings and counts), and a global IPv6 address inside a longer run of letters, digits or colons (`ip2001:db8::1`, `2001:db8::1x`) can't be recognised. Non-global IPv6 addresses (link-local `fe80::…`, unique-local `fd00::…`, loopback `::1`) stay, since they don't identify a connection publicly.
...
- The limits and GitHub's own rate limit, on the post or the app's sign-in: "You can suggest again later", with when. Nothing is recorded for GitHub's rate limit.
- GitHub didn't confirm the post (an outage, a timeout, an unreadable answer): "GitHub didn't confirm your suggestion". The try counts toward the limits, because the issue may exist; the card asks the member to check GitHub before sending it again. An outage during the app's sign-in reads and counts the same, although nothing was posted.
- A restart: a post already at GitHub finishes and records its row before the writer lease is handed over; one still waiting behind it gets "Please wait a moment" ("TaruBot is restarting right now.") and records nothing.
