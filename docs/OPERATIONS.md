...
- **When.** About 2 minutes after the first change, one `officer.status` job per server (key `officer:<guild>:status`) posts everything that changed in that window; a change undone inside it cancels out. A post names at most 100 members, fewer when their lines wouldn't fit, and further posts follow in the same run. While it waits, officers' `/sync status` lists `↻ WAITING officer.status 1a2b3c4d …` with the `ordered` diagnostic (logged at debug); members never see the job, since it has no user. A run with nothing to post completes as `{skipped: "nothing to post"}`, visible in `jobs.result` (`/sync status` lists no succeeded job). If the officer channel is unset or moved, or Discord changes are paused, while a run is posting, it stops before the next post (`superseded`) and the rerun drops, parks or posts to the new channel.
- **Not announced.** Decisions made while evidence is unconfirmed (an out-of-date roster, an unchecked new link, an unknown rank), until a fresh roster confirms them; hand edits the bot keeps in those times; role changes after `/config roles` or `/setup` binding changes, except officers adopted when a new Officer role replaces one already set; roles given on joining; people who left the server; nicknames; and the first pass after the deploy, activation or a new member row, which only records a baseline. Linking an FC to a server whose roles are set up posts its confirmed members as "Guest → Member" after the first roster, or about 2 minutes after the relink when the same FC is relinked while its last roster is still fresh (`/config fc unlink` keeps the membership rows, so the relink's repair pass is already decided); it is the mirror of the unlink post.
- **No channel.** While the officer notifications channel is unset, a change is taken as announced when it is recorded and queues nothing, and the roster records no departure: setting the channel later, even within 2 minutes, posts none of it. A job queued before the unset completes as `{skipped: "officer notifications unconfigured"}`, even with Discord changes off, and what waited counts as announced. With Discord changes paused and a channel set, it parks (`‖ PAUSED` for officers) and posts on resume.
- **Failures.** Missing permissions block the job (`! BLOCKED`) with its batch frozen. The scheduler requeues it about every 10 minutes, and a `/config` save or `retry.js` releases it at once; the retry resends the same batch under the same nonce (`status:<batch>`), so a retry within Discord's window returns the first message. Each send records a `started`, then a `failed` or a `delivered` delivery attempt; `delivered` holds the message ID and the nonce key in `diagnostic`, and is written before the batch is marked. If the mark then fails (a database error after Discord took the post), the job retries as usual and the retry only marks the batch, however late, so it never posts twice. A job that ends failed keeps what waits: `retry.js` or the next change in that server posts it.
