...
- **Failures and restarts.** A download or validation failure keeps the active set and logs `selectors_rejected` once per revision; a switch logs `selectors_updated` (from and to). A restarted container adopts the set it already activated.
