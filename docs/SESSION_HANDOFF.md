...
**Update, 2.28.0 (current version; issue #32; planned as 2.26.0):**
...
3. 2.24.2, 2.24.3 and 2.25.0 are deployed (2.25.0 on 2026-09-25 with migration 009 and `/config changelog` registered). After the next scheduled roster read, confirm production posted no "FC roster accepted" line. 2.27.0 (#33, the documentation site, PR #37) is merged and published; it needs no deploy. Then merge and deploy #32 (2.28.0, PR #36) and later #31 (2.29.0, PR #38), in the agreed release order. The robust, disposable host (owner decision, 2026-09-25) still needs the SSH deploy workflow from GitHub Actions, which takes the next free minor after #31; the heartbeat (2.22.0), rebuild runbook and settings copy (2.23.0) and daily backups (2.24.0) are done.
