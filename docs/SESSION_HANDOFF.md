...
- **Change.**
  ...
  - Docs: HOSTING.md "Automated deploys" (flow, outcomes, logs, the worker-died table, pause and kill, the owner's setup) and the rebuild runbook; CI_CD.md "Deploy production" with the itemized agent token; CLAUDE.md and AGENTS.md: the owner's GitHub approval is the go-ahead for production deploys, and Claude never approves, rejects or bypasses a deployment or holds the deploy key.
- **Deploy.** No bot change, no migration, no command change. DevBot needs nothing (GitHub can't reach the dev VM). Production gets 2.30.0 by hand once, as a restart, so the host has `ops/deploy.sh`; then the owner's setup ([HOSTING.md](HOSTING.md#setting-it-up-owner)) and a first run of 2.30.0, which should end `already-live`.
...
5. 2.29.1 is merged, so the next PR the app opens should get its Claude review with no close and reopen; record that run in VERIFICATION.md ([CI_CD.md](CI_CD.md#claude-review-and-assistant)).
