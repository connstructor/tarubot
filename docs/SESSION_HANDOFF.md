...
- **Merged with 2.27.0 and 2.28.0.** `main` at `c0a4f98` (2.25.0's merge, the documentation site as 2.27.0 and `/suggest` as 2.28.0) is merged in, and this release is renumbered 2.29.0. Main moved `docs/OPERATIONS.md`, `docs/SETUP.md` and the README's usage sections into the site, so this branch's edits to them moved too: what officers see (when the post comes, what it shows, what isn't announced, no channel, an example with placeholder names) went to `admin/notices-and-updates.md` "Member status changes"; the operators' side (the `officer:<guild>:status` job, outcomes, failures, the inspection query, the manual reversal and the lock order) to `deploy/monitoring.md` "Status notices"; the "Status notice" label and the post to `reference/replies.md`; and the command reference's `/config officer_notifications` names the posts. SETUP.md's upgrade note for migration 010 has no site page (the site describes the latest release): its facts are in the CHANGELOG entry. The old paths stay deleted. No member note (the posts go only to officers). The merged tree's container runs are still to do.
...
1. Read [../AGENTS.md](../AGENTS.md) and [../CLAUDE.md](../CLAUDE.md), inspect `git status`/history, and fetch remote state. Check the open PRs' CI and review state ([#36](https://github.com/deconfined/tarubot/pull/36), [#38](https://github.com/deconfined/tarubot/pull/38)).
...
3. 2.24.2, 2.24.3 and 2.25.0 are deployed (2.25.0 on 2026-09-25 with migration 009 and `/config changelog` registered). After the next scheduled roster read, confirm production posted no "FC roster accepted" line. 2.27.0 (#33, the documentation site, PR #37) is merged and published; it needs no deploy. #32 (2.28.0, PR #36) is merged as `c0a4f98`; deploy it, then merge and deploy #31 (2.29.0, PR #38: member status posts, migration 010, no registration), in the agreed release order. The robust, disposable host (owner decision, 2026-09-25) still needs the SSH deploy workflow from GitHub Actions, which takes the next free minor after #31; the heartbeat (2.22.0), rebuild runbook and settings copy (2.23.0) and daily backups (2.24.0) are done.
4. For 2.28.0 (#32, `/suggest`), with the owner's go-ahead: the app settings into the host's `.env`, the deploys (restarts), registration of 21 roots / 46 paths, the app probe and the owner's test `/suggest` ([OPEN_ITEMS.md](OPEN_ITEMS.md#production-after-the-cutover)); and the owner confirms "Public Bot" is off for the production application.
5. For 2.29.0 (#31, PR #38), after the merged tree's container runs, its merge and 2.28.0's deploy, with the owner's go-ahead: the migration procedure (migration 010, no registration) on DevBot ([plan](DEV_GUILD.md#2290-rollout-plan-member-status-posts-not-yet-run)) and production. The deploy posts nothing.
