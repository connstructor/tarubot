...
- **Local remote.** This clone's `origin` still points at `https://github.com/connstructor/tarubot.git` and works through the redirect. Changing it edits Git configuration, so with the owner's go-ahead run `git remote set-url origin https://github.com/deconfined/tarubot.git` (and the same in any other clone), then check it with `git remote -v` and `git fetch origin`.
...
4. With the owner, run the 2.15.0 session from [../test-plans/current.json](../test-plans/current.json) on the deployed 2.15.0 (plan message `1552666419064479837`). It checks every D1–D9 fix live and restores PigeonMuffin's officer access with `/officer reset`. Record message IDs, Refs, Codes and deviations in [DEV_GUILD.md](DEV_GUILD.md) and [OPEN_ITEMS.md](OPEN_ITEMS.md).
...
- [ ] **Replies (2.15.0):** the 2.15.0 session in `test-plans/current.json` on the deployed 2.15.0, including restoring PigeonMuffin's officer access with `/officer reset`.
