...
**Update, 2.30.0 (issue #41; current version):**
...
- **Change.**
  ...
  - Docs: HOSTING.md "Automated deploys" (flow, outcomes, logs, the worker-died table, pause and kill, the owner's setup) and the rebuild runbook; CI_CD.md "Deploy production" and "Agent access to deployments" (the guards in place and the owner's open decisions); REQUIREMENTS.md's canonical "Agent rule", which AGENTS.md carries verbatim and CLAUDE.md points to: the owner's GitHub approval is the go-ahead for production deploys, and agents never approve, reject, bypass, cancel or re-run a deployment, never enable or disable the workflow, and never hold the deploy key.
...
- **Status.** Branch `feat/ssh-deploy-2.30.0` from `4f3b377`; not pushed. The local checks pass ([VERIFICATION.md](VERIFICATION.md)); the first live runs are still to record.
...
1. Read [../AGENTS.md](../AGENTS.md) and [../CLAUDE.md](../CLAUDE.md), inspect `git status`/history, and fetch remote state. Check whether 2.30.0 (`feat/ssh-deploy-2.30.0`) was pushed and merged; 2.29.2 merged as PR #42 (`4f3b377`).
2. The owner closed W14 and W15, the DigitalOcean cleanup and the firewall on 2026-09-26 ([OPEN_ITEMS.md](OPEN_ITEMS.md#production-after-the-cutover)). Still the owner's: least privilege in Woven Souls (the owner asked for a full inventory of the bot's permissions to adjust by hand) and the 2.30.0 decisions below. The legacy MariaDB login is closed because the owner said in chat on 2026-09-25 that the legacy host is shut down; nothing records the login itself being rotated or dropped, so if the host is ever restarted, rotate or drop it first.
3. 2.24.2, 2.24.3, 2.25.0, 2.28.0 and 2.29.0 are deployed: 2.28.0 on 2026-09-25 with `/suggest` registered (21 roots / 46 paths), 2.29.0 on 2026-09-26 with migration 010 ([DEV_GUILD.md](DEV_GUILD.md#2290-rollout--2026-09-26)). 2.27.0 (#33, the documentation site) changed only documentation, so nothing was deployed for it, and 2.29.1 and 2.29.2 have nothing to deploy either. After the next scheduled roster read, confirm production posted no "FC roster accepted" line. The robust, disposable host (owner decision, 2026-09-25) gets its last part, the SSH deploy workflow, in 2.30.0; the heartbeat (2.22.0), rebuild runbook and settings copy (2.23.0) and daily backups (2.24.0) are done. Once 2.30.0 merges, walk the owner through the rest of its setup ([HOSTING.md](HOSTING.md#setting-it-up-owner)): prepare the commands only, follow the agent rule in AGENTS.md, and record the first runs in VERIFICATION.md.
...
5. Before 2.30.0 merges, the owner confirms the agent-rule clauses beyond question 1 and decides on signed build provenance and the agent's push credential ([OPEN_ITEMS.md](OPEN_ITEMS.md#production-after-the-cutover), [CI_CD.md](CI_CD.md#agent-access-to-deployments)). 2.29.1 is confirmed: PR #42's Claude review started by itself ([VERIFICATION.md](VERIFICATION.md)).
