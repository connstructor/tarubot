...
**Update, 2.29.1 (current version):**
- **Why.** The owner asked for the Claude review to start on the pull requests TaruBot's own GitHub App opens ("an easy win"). The job skipped every bot, so each of the agent's PRs was closed and reopened by hand to get a review.
- **Change.** `claude-code-review.yml` admits the sender `tarubot-agent[bot]` next to `User` senders, and the review step passes `allowed_bots: "tarubot-agent[bot]"`; without it the action fails a bot actor. The action checks no repository permission for a `[bot]` actor, so the list is that one name, never `*`, and every other bot stays skipped ([CI_CD.md](CI_CD.md#claude-review-and-assistant)). The 2.28.0 and 2.29.0 rollouts are recorded in DEV_GUILD.md and VERIFICATION.md. No bot change, no migration, nothing to deploy.
- **Status.** Branch `feat/review-bot-trigger-2.29.1` from `9d6e136`; not pushed. Its PR gets no Claude review, because it changes the workflow. After the merge, the next PR the app opens is the check: leave it alone (no close and reopen), and its run should log "Actor tarubot-agent[bot] is in allowed_bots list" and post the review.
...
1. Read [../AGENTS.md](../AGENTS.md) and [../CLAUDE.md](../CLAUDE.md), inspect `git status`/history, and fetch remote state. Check whether 2.29.1 (`feat/review-bot-trigger-2.29.1`) was pushed and merged; #38 merged as `9d6e136`.
...
3. 2.24.2, 2.24.3, 2.25.0, 2.28.0 and 2.29.0 are deployed: 2.28.0 on 2026-09-25 with `/suggest` registered (21 roots / 46 paths), 2.29.0 on 2026-09-26 with migration 010 ([DEV_GUILD.md](DEV_GUILD.md#2290-rollout--2026-09-26)). 2.27.0 (#33, the documentation site) changed only documentation, so nothing was deployed for it, and 2.29.1 has nothing to deploy either. After the next scheduled roster read, confirm production posted no "FC roster accepted" line. The robust, disposable host (owner decision, 2026-09-25) still needs the SSH deploy workflow from GitHub Actions, which takes the next free minor (2.30.0); the heartbeat (2.22.0), rebuild runbook and settings copy (2.23.0) and daily backups (2.24.0) are done.
4. Still to record from the rollouts, with the owner: the DevBot `/suggest` checks (2.28.0) and the status-post checks in `test-plans/current.json` (2.29.0) ([OPEN_ITEMS.md](OPEN_ITEMS.md#production-after-the-cutover)); and the owner confirms "Public Bot" is off for the production application.
5. After 2.29.1 merges, the next PR the app opens should get its Claude review with no close and reopen; record that run in VERIFICATION.md ([CI_CD.md](CI_CD.md#claude-review-and-assistant)).
