...
- **Status.** Merged ([PR #35](https://github.com/deconfined/tarubot/pull/35), `723274a`) and deployed on 2026-09-25 to DevBot (21:24 UTC) and production (21:25 UTC), both on schema `009_changelog_channel.sql`, with the commands registered (production `register.js --global`, 20 roots / 45 paths; DevBot's guild) ([DEV_GUILD.md](DEV_GUILD.md#2250-rollout--2026-09-25)). Production has no changelog channel yet, so nothing was posted. The branch had 2.24.3 (`ef0893b`) merged in. One review round is folded in (the member note's wording, the repeat receipt's Visibility field, the paused no-post test). Before that merge, the full container run with the supplied dump passed **1,316 tests / 38,033 assertions** after one test fix (migration 006's rehearsal now inserts its later guild with raw SQL, since today's Drizzle mapping names 009's columns). On the merged tree, the full container run passed **1,321 tests / 38,106 assertions** with the supplied dump and with the synthetic fixture (2.24.3's five PostgreSQL scenarios added); [VERIFICATION.md](VERIFICATION.md) has the evidence.
...
- **Release order.** 2.24.3 (#29) and 2.25.0 (#30) are deployed, and 2.27.0 (#33) is merged (documentation only). #31 follows as 2.29.0 ([PR #38](https://github.com/deconfined/tarubot/pull/38), migration 010). Deploying 2.28.0 is a restart plus the app settings in the host's `.env`, registration of 21 roots / 46 paths and a probe ([DEV_GUILD.md](DEV_GUILD.md#2280-rollout-plan-suggest-not-yet-run)).

**Update, 2.29.0 (current version; issue #31, PR #38; built as 2.27.0):**
...
- **Change.**
  ...
  - With no officer channel, a change counts as announced when it is recorded and no departure is recorded, so nothing posts after the channel is set.
...
1. Read [../AGENTS.md](../AGENTS.md) and [../CLAUDE.md](../CLAUDE.md), inspect `git status`/history, and fetch remote state. Check the open PR's CI and review state ([#38](https://github.com/deconfined/tarubot/pull/38)); #36 merged as `c0a4f98`.
...
3. 2.24.2, 2.24.3 and 2.25.0 are deployed (2.25.0 on 2026-09-25 with migration 009 and `/config changelog` registered). After the next scheduled roster read, confirm production posted no "FC roster accepted" line. 2.27.0 (#33, the documentation site, PR #37) is merged and published; it needs no deploy. #32 (2.28.0, PR #36) is merged as `c0a4f98` and published (run 36202103980); its rollout was under way at 23:49 UTC (DevBot on the 2.28.0 image), so read its record first and finish only what it leaves. Then merge and deploy #31 (2.29.0, PR #38: member status posts, migration 010, no registration), in the agreed release order. The robust, disposable host (owner decision, 2026-09-25) still needs the SSH deploy workflow from GitHub Actions, which takes the next free minor after #31; the heartbeat (2.22.0), rebuild runbook and settings copy (2.23.0) and daily backups (2.24.0) are done.
4. For 2.28.0 (#32, `/suggest`), whatever its rollout record doesn't show as done, with the owner's go-ahead: the app settings into the host's `.env`, the deploys (restarts), registration of 21 roots / 46 paths, the app probe and the owner's test `/suggest` ([OPEN_ITEMS.md](OPEN_ITEMS.md#production-after-the-cutover)); and the owner confirms "Public Bot" is off for the production application.
5. For 2.29.0 (#31, PR #38; the merged tree's container runs passed), after its push with an updated title and body, its merge and the end of 2.28.0's rollout, with the owner's go-ahead: the migration procedure (migration 010, no registration) on DevBot ([plan](DEV_GUILD.md#2290-rollout-plan-member-status-posts-not-yet-run)) and production. The deploy posts nothing.
6. After that, OPS-10 and the rest of OPS-11 (repeated delivery-failure summaries).
