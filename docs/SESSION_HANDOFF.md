...
- **Status.** Branch `feat/changelog-posts-2.25.0`; not yet pushed. One review round is folded in (the member note's wording, the repeat receipt's Visibility field, the paused no-post test). The full container run with the supplied dump passed **1,316 tests / 38,033 assertions** after one test fix (migration 006's rehearsal now inserts its later guild with raw SQL, since today's Drizzle mapping names 009's columns); [VERIFICATION.md](VERIFICATION.md) has the evidence.
