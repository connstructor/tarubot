...
4. For 2.26.0 (#32, `/suggest`), with the owner's go-ahead: the app settings into the host's `.env`, the deploys, registration, the app probe and the owner's test `/suggest` ([OPEN_ITEMS.md](OPEN_ITEMS.md#production-after-the-cutover)); and the owner confirms "Public Bot" is off for the production application.
