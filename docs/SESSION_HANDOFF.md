...
- **Status.** Branch `feat/changelog-posts-2.25.0`, [PR #35](https://github.com/deconfined/tarubot/pull/35), merged into `main` as `723274a` on 2026-09-25 and published; its DevBot and production rollout is not yet recorded. It had 2.24.3 (`ef0893b`) merged in. One review round is folded in (the member note's wording, the repeat receipt's Visibility field, the paused no-post test). Before that merge, the full container run with the supplied dump passed **1,316 tests / 38,033 assertions** after one test fix (migration 006's rehearsal now inserts its later guild with raw SQL, since today's Drizzle mapping names 009's columns). On the merged tree, the full container run passed **1,321 tests / 38,106 assertions** with the supplied dump and with the synthetic fixture (2.24.3's five PostgreSQL scenarios added); [VERIFICATION.md](VERIFICATION.md) has the evidence.
...
- **Status.** Branch `feat/docs-site` from `4a64609`, open as [PR #37](https://github.com/deconfined/tarubot/pull/37). It **merges after #36** (2.26.0, #32): merge `main` again once #36 is in and port #32's docs into the site. [PR #38](https://github.com/deconfined/tarubot/pull/38) (#31) is also on 2.27.0, so whichever merges second takes the next free minor. **Merge the PR with a merge commit, not squash:** the first commit is the pure `git mv`, and a squash would lose the `git log --follow` history of the moved guides. GitHub Pages is already enabled (source GitHub Actions, `github-pages` limited to `main`).
...
1. Read [../AGENTS.md](../AGENTS.md) and [../CLAUDE.md](../CLAUDE.md), inspect `git status`/history, and fetch remote state. Check the open PRs' CI and review state ([#36](https://github.com/deconfined/tarubot/pull/36), [#37](https://github.com/deconfined/tarubot/pull/37), [#38](https://github.com/deconfined/tarubot/pull/38)).
...
3. 2.24.2 and 2.24.3 are deployed. After the next scheduled roster read, confirm production posted no "FC roster accepted" line. Then deploy 2.25.0 (#30, PR #35, merged as `723274a`: update posts, migration 009, a command registration), and merge and deploy #32 (2.26.0) and later #31 (2.27.0), in the agreed release order. PR #37 (#33, the documentation site, no deploy) merges after #36; it and #38 both carry 2.27.0 on their branches, so the second to merge renumbers. The robust, disposable host (owner decision, 2026-09-25) still needs the SSH deploy workflow from GitHub Actions, which takes the next free minor after #31; the heartbeat (2.22.0), rebuild runbook and settings copy (2.23.0) and daily backups (2.24.0) are done.
