...
- **Release order** (approved on #47): **2.30.1** (#48, the cleanup patch, branch `chore/cleanup-2.30.1`), then **2.30.2** (#47, Discord channel obfuscation, deployed before it becomes mandatory on 2026-11-16), then **2.31.0** (#46, `/setup` overrides, a dry run by default with `confirm:true` to write and run while TaruBot still holds Administrator, and a role and visibility check in `/config validate`: the path to removing Administrator in production).
...
**Update, 2.30.1 (issue #48; current version):**
...
- **Status.** Branch `chore/cleanup-2.30.1` from `853c4f3`. All checks pass locally ([VERIFICATION.md](VERIFICATION.md#automated-suites)): unit 1,419, contract 34, Docker 1,568 with each fixture, the site build and `ci:version`.
...
1. Read [../AGENTS.md](../AGENTS.md) and [../CLAUDE.md](../CLAUDE.md), inspect `git status`/history, and fetch remote state. Check whether 2.30.1 (#48, `chore/cleanup-2.30.1`) merged and whether its Deploy production run went out; record the run in VERIFICATION.md and DEV_GUILD.md, and update DevBot by hand with @deconfined's go-ahead.
...
3. Next release: **2.30.2** (#47), Discord channel obfuscation, deployed before 2026-11-16. DevBot's toggle is on for testing ([DEV_GUILD.md](DEV_GUILD.md#channel-obfuscation-test-toggle--2026-09-26)). Then **2.31.0** (#46), the `/setup` overrides and the `/config validate` role and visibility check, after which @deconfined can take Administrator away in Woven Souls.
...
- [ ] **2.30.1 (#48):** the cleanup patch; merge, deploy through Deploy production with @deconfined's approval, and update DevBot.
- [ ] **2.30.2 (#47):** Discord channel obfuscation, deployed before 2026-11-16.
...
- [ ] Least privilege in Woven Souls: Administrator stays on until 2.31.0's `/setup` overrides have run and `/config validate` says removing it is safe.
