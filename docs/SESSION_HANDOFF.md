...
- **Status.** Branch `feat/changelog-posts-2.25.0`, with 2.24.3 (`ef0893b`) merged in, merged into `main` in [PR #35](https://github.com/deconfined/tarubot/pull/35) as `723274a` on 2026-09-25; not yet deployed. One review round is folded in (the member note's wording, the repeat receipt's Visibility field, the paused no-post test). Before that merge, the full container run with the supplied dump passed **1,316 tests / 38,033 assertions** after one test fix (migration 006's rehearsal now inserts its later guild with raw SQL, since today's Drizzle mapping names 009's columns). On the merged tree, the full container run passed **1,321 tests / 38,106 assertions** with the supplied dump and with the synthetic fixture (2.24.3's five PostgreSQL scenarios added); [VERIFICATION.md](VERIFICATION.md) has the evidence.

**Update, 2.27.0 (current version):**
...
- **Deploy.** The migration procedure (stop, back up, migrate, `up`); no registration; nothing posts on the deploy. There is no image rollback across migration 010 except the manual reversal in OPERATIONS.md "Status notices". [DEV_GUILD.md](DEV_GUILD.md#2270-rollout-plan-member-status-posts-not-yet-run) has DevBot's plan.
- **Status.** Branch `feat/status-notices-2.27.0`, based on `f2c11d2`, the tree [PR #35](https://github.com/deconfined/tarubot/pull/35) merged into `main` as `723274a` (same content; `ci:version` prints "Validated release version 2.27.0 above 2.25.0."). One review round is folded in: the lock order (guild row first, `FOR NO KEY UPDATE` member locks), the no-channel rule at recording time, a failed mark after the send, the drain's settings check, the lost-Guest reason and doc corrections. The fast checks pass (1,207 unit, 26 contract tests), and the full container run passed **1,380 tests / 38,989 assertions** with the supplied dump and with the synthetic fixture, repeated on the committed `a56f72c` with the same totals ([VERIFICATION.md](VERIFICATION.md)). #32 (2.26.0, no migration) merges before it in the release order; then this branch rebases onto `main`, with 2.26.0's CHANGELOG entry between 2.27.0 and 2.25.0, before its PR.
...
1. Read [../AGENTS.md](../AGENTS.md) and [../CLAUDE.md](../CLAUDE.md), inspect `git status`/history, and fetch remote state. Check the open PRs' CI and review state (#32's 2.26.0 first; PR #35, 2.25.0, merged as `723274a`).
...
3. 2.24.2 and 2.24.3 are deployed. After the next scheduled roster read, confirm production posted no "FC roster accepted" line. Then deploy 2.25.0 (#30, merged in PR #35 as `723274a`: update posts, migration 009, a command registration), merge and deploy #32 (2.26.0), and later #31 (2.27.0, branch `feat/status-notices-2.27.0`: member status posts, migration 010, no registration; rebase onto `main` after #32 merges, then open its PR), in the agreed release order. The robust, disposable host (owner decision, 2026-09-25) still needs the SSH deploy workflow from GitHub Actions, which takes the next free minor after #31; the heartbeat (2.22.0), rebuild runbook and settings copy (2.23.0) and daily backups (2.24.0) are done.
4. After that, OPS-10 and the rest of OPS-11 (repeated delivery-failure summaries).
