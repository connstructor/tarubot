...
**Update, 2.26.0 (current version; issue #32):**
...
- **Status.** Branch `feat/suggest-2.26.0`, open as [PR #36](https://github.com/deconfined/tarubot/pull/36), with 2.25.0 (`723274a`, PR #35) merged in (`9ac66c7`), so the version sentences in CONFIGURATION.md and PERSISTENCE.md now require `009_changelog_channel.sql` and 2.26.0 still adds no migration. 2.26.0 has a member note for update posts (`197c55c`). On the merged tree the fast checks pass and the full container run passed **1,411 tests / 63,115 assertions** with the supplied dump and with the synthetic fixture, after the /suggest scenarios moved to their own guild IDs (`0f60ddc`) ([VERIFICATION.md](VERIFICATION.md)). 2.24.3 (#29) and 2.25.0 (#30), which come before it in the agreed release order, are deployed; #31 is 2.27.0. Deploying it is a restart plus the app settings in the host's `.env`, registration of 21 roots / 46 paths and a probe ([DEV_GUILD.md](DEV_GUILD.md#2260-rollout-plan-suggest-not-yet-run)).
...
1. Read [../AGENTS.md](../AGENTS.md) and [../CLAUDE.md](../CLAUDE.md), inspect `git status`/history, and fetch remote state. Check [PR #36](https://github.com/deconfined/tarubot/pull/36)'s CI and review state.
