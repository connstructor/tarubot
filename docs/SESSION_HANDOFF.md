...
**Snapshot: 2026-09-23, updated 2026-09-26 after the 2.30.1 merge. Start here next session.** This records the observed repository, publication, and DevBot state; recheck them before making changes. [OPEN_ITEMS.md](OPEN_ITEMS.md) is the detailed requirements-backed checklist, and the documentation site's [roadmap](../site/src/content/docs/project/roadmap.md) records the owner's v2–v6 plan.

## 1. Where we stopped

**Current state (2026-09-26):**
- **Production** runs **2.30.0** on schema `010_status_notices.sql` on the production host (a Linode Docker host with Linode managed PostgreSQL; [HOSTING.md](HOSTING.md)). It was deployed by hand on 2026-09-26 (healthy at 12:37:39 UTC). Since then deploys are automated: the Deploy production workflow runs each published release once @deconfined approves the `production` environment in GitHub, with `DEPLOY_ENABLED` a repository variable. The first automated run (36242986952) ended `already-live` ([DEV_GUILD.md](DEV_GUILD.md#2300-rollout--2026-09-26)). Administrator stays on in Woven Souls until #46 (below).
- **DevBot** runs **2.30.0** on schema 010 (healthy at 12:36:35 UTC), with Discord's Private Channel Obfuscation toggle on in its Developer Portal for testing, restarted at 13:34:49 UTC ([DEV_GUILD.md](DEV_GUILD.md#channel-obfuscation-test-toggle--2026-09-26)).
...
- **Deploy.** No migration and no command change: a restart on each. DevBot and production before 2026-11-16, then DevBot's read-only check on or just after the date ([DEV_GUILD.md](DEV_GUILD.md#2302-channel-obfuscation-checks-47--pending)). If the date gets close first, the owner can give DevBot's role View Channel on the `Admin` category and in `#moderator-only`; both stay excluded by ID.
- **Status.** Branch `fix/obfuscation-2.30.2` from `ea7212e` (2.30.1). All checks pass locally ([VERIFICATION.md](VERIFICATION.md#automated-suites)): typecheck, lint, format and build; unit 1,439 across 61 files; contract 34; Docker 1,589 (40 skipped) with the supplied fixture and again with the synthetic one; `ci:version` (2.30.2 above 2.30.1); and the site build (35 pages, links valid).
...
1. Read [../AGENTS.md](../AGENTS.md) and [../CLAUDE.md](../CLAUDE.md), inspect `git status`/history, and fetch remote state. 2.30.1 (#48) merged as PR #49 (`ea7212e`): check whether its Deploy production run went out; record the run in VERIFICATION.md and DEV_GUILD.md, and update DevBot by hand with @deconfined's go-ahead.
...
3. Next release: **2.30.2** (#47), Discord channel obfuscation, in progress on `fix/obfuscation-2.30.2`. Deploy it before 2026-11-16 and run DevBot's checks, including the read-only one on or just after the date ([DEV_GUILD.md](DEV_GUILD.md#2302-channel-obfuscation-checks-47--pending)); DevBot's toggle stays on until then. Then **2.31.0** (#46), the `/setup` overrides and the `/config validate` role and visibility check, after which @deconfined can take Administrator away in Woven Souls.
4. Raise @deconfined's open items ([OPEN_ITEMS.md](OPEN_ITEMS.md#owner-checks-owed)): the DevBot checks owed for 2.25.0, 2.28.0 and 2.29.0, to run or waive; the documentation site's search, 404 page and edit links; "Public Bot" off for the production application, and both applications' Developer Portal settings as documented; the two open 2.30.0 review decisions (signed build provenance, and the agent's push credential; [CI_CD.md](CI_CD.md#agent-access-to-deployments)); the v3 plan's answers on #43; and the staging host. The legacy MariaDB login is closed because the legacy host is shut down; if the host is ever restarted, rotate or drop the login first.
...
- [ ] **2.30.1 (#48):** the cleanup patch, merged (PR #49, `ea7212e`); deploy through Deploy production with @deconfined's approval, and update DevBot.
- [ ] **2.30.2 (#47):** Discord channel obfuscation, on `fix/obfuscation-2.30.2`; merge, deploy to DevBot and production before 2026-11-16, and run DevBot's check on or just after the date.
- [ ] **2.31.0 (#46):** `/setup` overrides and the `/config validate` role and visibility check, then Administrator off in Woven Souls.
...
- [ ] A staging host (@deconfined is creating a second Linode; a staging issue follows).
...
- one per later DevBot rollout, named in [DEV_GUILD.md](DEV_GUILD.md), through `.cache/backups/tarubot_dev-before-2.30.0-853c4f3.dump` (schema 010)
...
> Read AGENTS.md, CLAUDE.md, and docs/SESSION_HANDOFF.md. TaruBot v2 has been live in Woven Souls since 2026-09-24. It runs on the production host, a Linode Docker host, with Linode managed PostgreSQL (docs/HOSTING.md). Production and DevBot run 2.30.0 on schema 010, and production deploys run through the Deploy production workflow once @deconfined approves them; follow the agent rule in AGENTS.md. 2.30.1 (#48, the cleanup patch) merged as PR #49: check whether it was deployed, record its run, and update DevBot with @deconfined's go-ahead. Then finish 2.30.2 (#47, Discord channel obfuscation, branch fix/obfuscation-2.30.2; deploy before 2026-11-16) and plan 2.31.0 (#46, `/setup` overrides and the `/config validate` role and visibility check). Raise the owner's open items in docs/OPEN_ITEMS.md. Preserve the owner's decisions in REQUIREMENTS.md.
