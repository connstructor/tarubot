...
- **Change.**
  ...
  - Migration `010_status_notices.sql`: `guild_users.status_state`, `status_since`, `status_posting`. The roster records departures in its snapshot transaction with a fixed lock order, and `sync.guild` locks its rows up front in that order.
  - The `officer.status` job resends a frozen batch first, waits out the window, then freezes budget-sized batches under its lease and marks each right after the send. No channel drops what waits. The queue resets `attempts` on a generation requeue.
...
- **Status.** Branch `feat/status-notices-2.27.0`, built on 2.25.0's branch (PR #35, which carries 2.24.3; not yet merged). The fast checks pass (1,206 unit, 26 contract tests); the full container run, which runs the new PostgreSQL scenarios, is still to do ([VERIFICATION.md](VERIFICATION.md)). #32 (2.26.0, no migration) merges before it in the release order.
