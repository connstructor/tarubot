...
- **Claude Code Review** (`claude-code-review.yml`) reviews ready, same-repository PRs to `main` with the code-review plugin and posts inline comments. It skips drafts, fork PRs, and runs started by bots, and a newer push cancels an unfinished review. It is advisory, never a required check. Its full transcript is temporarily printed to the public Actions log (`show_full_output`) to diagnose silent reviews; remove that input once the cause is known. It also skips, reporting success, on a PR that changes the workflow file itself, because the action only runs a workflow identical to the default branch's copy.
