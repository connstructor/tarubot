...
The refusals: outside the deployment's own server, `forbidden.context` ("Suggestions can be sent only from the Free Company server this TaruBot serves."), and the same concept when suggestions are switched off. Without the Member or Guest role, `forbidden.membership` with the lead "Only members and guests of this server can suggest features." and steps to either role. An unconfigured server gets the setup card. An idea under 10 characters once invisible characters are removed is an input failure with the command's Example. The limits and GitHub's rate limit are `wait.suggest`; an unconfirmed post is `upstream.github`. A refused GitHub App credential, or a request GitHub rejects, is `unexpected` with a private issue report.
