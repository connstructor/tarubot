...
export class DiscordGuildAccess implements GuildAccessPort {
  ...
  /** The guild endpoint covers every non-thread channel type; threads inherit their parent's gate. */
  ...
  async prepare(
    ...
    const officers = await ensure(existingOfficers, "officers", "officers");
  ...
  /** Fail closed for future default channels, while the lobby's explicit allow remains visible. */
