...
- Officer Lodestone notices (2.24.3, #29):
  ...
  - An accepted roster with `last_error` set closes the key's unstarted rows as `{skipped: "recovered before posting"}`. It queues `officer:<guild>:recovered:<fc>` (5 s) only if a degraded row created after the previous boundary has a `message_id` or is `running`. `/config fc unlink` closes the old FC's unstarted row as `{skipped: "FC unlinked"}`.
