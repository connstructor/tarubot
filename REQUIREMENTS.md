...
**Command surface.** `/suggest` brings the command surface to 21 roots and 46 paths (AC-23), after 2.25.0's `/config changelog`. It must be registered after the deployment. Release order (agreed 2026-09-25): #29 as 2.24.3, #30 as 2.25.0, #32 as 2.26.0, then #31 as 2.27.0.
...
| AC-23 | The deployed command inventory matches Section 4, and the bot operates with the intents and explicit permissions specified in OPS-12 (21 root commands / 46 paths since 2.26.0, including `/config role_layout`, `/officer reset`, `/guest reset`, `/issue`, `/config changelog`, and `/suggest`). |
