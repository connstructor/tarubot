...
On 2026-09-22 the owner required ongoing tracking of the latest upstream Nodestone code and CSS selectors. Their manifest references follow upstream HEAD independently. A checked update workflow advances the lockfile/build metadata and rebuilds the sidecar; deployed images retain exact revision identities for reproducibility. The sidecar periodically reports upstream freshness so parser dependencies are not silently left on old revisions.
