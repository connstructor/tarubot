...
- **Waiting.** While the window runs, officers' `/sync status` lists `↻ WAITING officer.status 1a2b3c4d …` with the `ordered` diagnostic, logged at debug. Members never see the job, since it has no member.
- **Outcomes.** A post succeeds with its message. A run with nothing left to post completes as `{"skipped": "nothing to post"}`; `/sync status` lists no succeeded job, so read it in `jobs.result`. With no officer notifications channel, the job completes as `{"skipped": "officer notifications unconfigured"}`, even with Discord changes off, and whatever waited counts as announced: nothing is saved for a channel set later. With a channel and Discord changes paused, it parks as `‖ PAUSED` and posts on resume.
...
- **A failed job** keeps what waits: [`retry.js`](/tarubot/deploy/tools/#retryjs), or the next change in that server, posts it.
