...
2. **Copy the application ID.** On **General Information**, copy the **Application ID**. It goes in `.env` as `DISCORD_APPLICATION_ID`, and in the invite link as `YOUR_APPLICATION_ID`.
...
5. **Decide who can add the bot.** Leaving **Public Bot** on lets anyone with the invite link add it to a server they manage. Turn it off to keep the deployment to your own servers.
...
Next, [install the deployment](/tarubot/deploy/install/), then [add the bot to your server](/tarubot/admin/add-to-server/) with the invite link.
